package frc.robot.state.vision;

import edu.wpi.first.math.geometry.Pose3d;
import edu.wpi.first.math.geometry.Rotation3d;
import edu.wpi.first.math.geometry.Translation3d;
import edu.wpi.first.math.util.Units;

public class VisionConstants {
    public static final double fieldBorderMargin = 0.25; // meters
    public static final double zMargin = 0.5; // meters
    public static final double xyStdDevCoefficient = 0.05; // was 0.005 meters
    public static final double thetaStdDevCoefficient = 0.05; // radians?
    public static final double[] stdDevFactors = new double[] { 1.0, 1.0 };

    private static final double x_offset = Units.inchesToMeters(0.0);
    private static final double y_offset = Units.inchesToMeters(0.0);
    private static final double z_offset = Units.inchesToMeters(0.0);

    private static final double roll = Units.degreesToRadians(0.0);
    private static final double pitch = Units.degreesToRadians(0.0);
    private static final double yaw = Units.degreesToRadians(0.0);

    public static final Pose3d[] robotToCameraPoses = new Pose3d[] {
            // Camera 1 - left
            new Pose3d(
                    new Translation3d(
                            x_offset,
                            y_offset,
                            z_offset),
                    new Rotation3d(
                            roll,
                            pitch,
                            yaw)),
            // Camera 2 - right
            new Pose3d(
                        new Translation3d(
                            x_offset,
                            y_offset,
                            z_offset),
                        new Rotation3d(
                                roll,
                                pitch,
                                yaw)),
    };

    public static final Pose3d[] cameraToRobotPoses = new Pose3d[] {
            // Camera 1 - left
            new Pose3d(
                    new Translation3d(
                            x_offset,
                            y_offset,
                            z_offset),
                    new Rotation3d(
                            roll,
                            pitch,
                            yaw)),
            // Camera 2 - right
            new Pose3d(
                        new Translation3d(
                            x_offset,
                            y_offset,
                            z_offset),
                        new Rotation3d(
                                roll,
                                pitch,
                                yaw)),
    };

    public static final String[] instanceNames = { "left", "right" };
}
